...
trait Optimizer extends py.Object {
  ...
  def minimize(loss: py.Any, var_list: PythonSeq[Variable]): Operation = py.native

  def apply_gradients(grads_and_vars: PythonSeq[(Tensor, Variable)]): Operation = py.native
}
